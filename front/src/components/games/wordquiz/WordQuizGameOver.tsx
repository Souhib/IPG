...
interface WordQuizGameOverProps {
  winner: string | null
...
export const WordQuizGameOver = memo(function WordQuizGameOver({
  winner,
  leaderboard,
  onBackToRoom,
  onLeaveRoom,
}: WordQuizGameOverProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {winner && (
        ...
        >
          ...
          <h2 className="text-xl font-extrabold tracking-tight gradient-text">
            {t("game.wordQuiz.winner", { username: winner })}
          </h2>
        </motion.div>
      )}
      ...
      <div className="glass rounded-2xl border-border/30 p-5">
        <h3 className="text-sm font-extrabold tracking-tight mb-4">{t("game.wordQuiz.finalScores")}</h3>
